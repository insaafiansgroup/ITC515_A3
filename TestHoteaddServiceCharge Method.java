...
public class TestHotel {
    ...
    public void setUp() throws Exception {
		...
		hotel.addRoom(RoomType.DOUBLE, 302);
	...
	public void testAddServiceCharge() { 
		...
		long confirmationNumber = hotel.book(room, guest, date, stayLength, 3, card);

		hotel.checkin(confirmationNumber);

		hotel.addServiceCharge(room.getId(), ServiceType.RESTAURANT, 8.00);
		...
		boolean isServicedded = true;

		for (ServiceCharge serviceCharge : booking.getCharges()) {

			if (serviceCharge.getType() == ServiceType.RESTAURANT) {
